...
forText_cellsFiltered <- forText_cellsFiltered2 <- forText_genesLost <- forText_genesLost2 <- NA
forText_dropMito <- forText_mitoMads <- forText_genesRemain <- forText_expFilt <- forText_mitoPct <- NA
forText_dropNorm <- forText_g2m <- forText_genesLost3 <- forText_libSizeP <- NA
...
eb_raw <- eb_raw[,colSums(eb_raw) > 5000]
...
forText_libSizeP <- 1e-3
tB <- with(cS,geneDetect < (libSize * 0.15 + 50))
tD <- cS$libSize > qnbinom(forText_libSizeP,size=fitLibSize$estimate["size"],
                           mu=fitLibSize$estimate["mu"],lower.tail=F)
eb_rawF1 <- eb_raw[,!tD]
eb_rawF1 <- eb_rawF1[rowSums(eb_rawF1) > 0,]
eb_rawF1 <- eb_rawF1[,colSums(eb_rawF1) > 200]
forText_cellsFiltered <- ncol(eb_raw) - ncol(eb_rawF1)
forText_genesLost <- nrow(eb_raw) - nrow(eb_rawF1)
rm(list=ls()[grepl("temp",ls())])
rm(eb_raw)
tB1 <- tB[!tD]
gsOut <- data.frame(DR=apply(eb_rawF1[,tB1],1,function(X) sum(X > 0)/length(X)),
                    MDTC=apply(eb_rawF1[,tB1],1,function(X) mean(X[X>0])),
                    MTC=rowMeans(eb_rawF1[,tB1]))
gsIn <- data.frame(DR=apply(eb_rawF1[,!tB1],1,function(X) sum(X > 0)/length(X)),
                   MDTC=apply(eb_rawF1[,!tB1],1,function(X) mean(X[X>0])),
                   MTC=rowMeans(eb_rawF1[,!tB1]))
tempMGC <- 1e2
eb_rawF <- eb_rawF1[,!tB1]
eb_rawF <- eb_rawF[rowSums(eb_rawF) > 0,]
forText_cellsFiltered2 <- ncol(eb_rawF1) - ncol(eb_rawF)
forText_genesLost2 <- nrow(eb_rawF1) - nrow(eb_rawF)
rm(eb_rawF1)
eb1p <- newSCESet(countData=eb_rawF)
eb1p <- calculateQCMetrics(eb1p,feature_controls=list(Mt=grepl("^MT-",rownames(eb1p))))
...
forText_genesLost3 <- nrow(eb1p) - nrow(eb1)
...
forText_g2m <- percent(sum(cycScores$phases == "G2M")/length(cycScores$phases))
...
if (!file.exists("e15_eb1S.RData")) {
  ...
  save(eb1S,maxPCt,minDEgenes,uniqDE,pDat,file="e15_eb1S.RData")
